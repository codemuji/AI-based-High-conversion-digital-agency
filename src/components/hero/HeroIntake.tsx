...
export function HeroIntake({ onStartOnboarding }: HeroIntakeProps) {
  ...
  return (
    <section ref={containerRef} id="hero" className="relative py-16 md:py-24 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto text-center overflow-hidden">
      {/* Background signature motif */}
      <div className="absolute inset-0 bg-grid-pattern -z-10 opacity-60 mask-radial" />

      {/* Main Headline */}
      <h1 ref={headlineRef} className="font-display text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black tracking-tight text-[var(--foreground)] w-full max-w-none lg:max-w-6xl mx-auto leading-[1.08]">
        <span className="sm:whitespace-nowrap">Custom software engineered to scale.</span> <br className="hidden sm:inline" />
        <span className="relative inline-block sm:whitespace-nowrap text-[var(--foreground)] font-extrabold">
          Scoped &amp; priced in 60 seconds.
          <span className="absolute left-0 -bottom-1.5 w-full h-1.5 bg-[var(--accent)] rounded-full opacity-90" />
        </span>
      </h1>

      <p ref={subheadRef} className="mt-7 text-base sm:text-lg text-[var(--muted)] max-w-2xl mx-auto leading-relaxed">
        Describe your vision below—our AI Architect instantly calculates exact pricing, tech stack, and delivery timeline in 4 simple questions.
      </p>

      {/* Live Status Trust Badges */}
      <div ref={badgesRef} className="mt-8 flex flex-wrap items-center justify-center gap-2.5 sm:gap-3 text-xs font-mono uppercase tracking-wider text-[var(--muted)]">
        <span className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-[var(--surface)] border border-[var(--surface-border)] shadow-2xs">
          <span className="w-2 h-2 rounded-full bg-green-600 animate-pulse" />
          <span className="font-bold text-[var(--foreground)]">Active AI Triage</span>
        </span>
        <span className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-[var(--surface)] border border-[var(--surface-border)] shadow-2xs">
          <span>⚡ 60-Sec Scoping</span>
        </span>
        <span className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-[var(--surface)] border border-[var(--surface-border)] shadow-2xs">
          <span>🛡️ No Sales Calls Required</span>
        </span>
      </div>

      {/* Main Search/Chat Intake Bar */}
      <form ref={searchRef} onSubmit={handleSubmit} className="mt-4 max-w-3xl mx-auto">
        <div className="relative flex items-center p-2 rounded-2xl bg-[var(--surface)] border border-[var(--surface-border)] hover:border-stone-400 focus-within:border-[var(--foreground)] focus-within:ring-2 focus-within:ring-[var(--foreground)]/10 shadow-md hover:shadow-lg transition-all duration-200">
          ...
          <button
            type="submit"
            className="shrink-0 px-6 sm:px-8 py-3.5 rounded-xl bg-[var(--foreground)] hover:bg-stone-800 text-white font-display font-semibold text-base shadow-sm active:scale-[0.98] transition-all duration-150 flex items-center gap-2 cursor-pointer"
          >
