        ...
        {paginatedPosts.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {paginatedPosts.map((post) => (
              ...
              >
                <div>
                  {/* Cover Image or Professional Placeholder Header */}
                  {(() => {
                    let cardImg = post.coverImage && (post.coverImage.startsWith("/blog-images/") || post.coverImage.startsWith("/images/")) ? post.coverImage : null;
                    if (!cardImg && post.content) {
                      const match = post.content.match(/<img[^>]+src=["']([^"']+)["']/i);
                      if (match && match[1] && (match[1].startsWith("/blog-images/") || match[1].startsWith("/images/"))) {
                        cardImg = match[1];
                      }
                    }

                    return cardImg ? (
                      <div className="relative w-full h-48 sm:h-52 bg-stone-100 overflow-hidden border-b border-stone-200">
                        <Image
                          src={cardImg}
                          alt={post.title}
                          fill
                        ...
                        />
                        <div className="absolute top-4 left-4 px-3 py-1 rounded-full bg-black/75 backdrop-blur-md text-white font-mono text-[10px] font-bold uppercase tracking-wider shadow-md">
